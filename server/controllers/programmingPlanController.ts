...
import { programmingSubPlanIdsIsRequired } from 'maestro-shared/schema/User/User';
...
export const programmingPlanRouter = {
  ...
  '/programming-plans/:programmingPlanId': {
    get: async ({ user, userRole }, { programmingPlanId }) => {
      ...
      if (
        ...
        !intersection(
          ...
          programmingPlan.subPlanIds
        ).length
      ) {
        return { status: HttpStatus.FORBIDDEN };
      }

      //TODO mettre au niveau sous plans
      ...
      //     : programmingPlan.subPlans.map((subPlan) => ({
      //         ...subPlan,
      //         departmentalStatus: subPlan.departmentalStatus.filter(
      //           (_) =>
      //             userStatusAuthorized.includes(_.status) &&
      //             userRegionsForRole(user, userRole).includes(_.region) &&
    ...
    // TODO à mettre au niveau sous plan
