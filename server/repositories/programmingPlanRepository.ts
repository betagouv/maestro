...
export const ProgrammingPlanNationalCoordinators = (transaction = db) =>
  transaction<{ programmingPlanId: string; userId: string }>(
    programmingPlanNationalCoordinatorsTable
  );
...
const parseProgrammingPlan = (
  ...
  ProgrammingPlanChecked.parse({
    ...row,
    nationalStatus: row.nationalStatus?.[0]
  });
