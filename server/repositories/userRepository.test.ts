...
test("impossible d'avoir 2 utilisateurs avec le même email", async () => {
  ...
  await expect(async () =>
   kysely.insertInto('users').values(genUser({
    email,
...
test("peut modifier le nom et le prénom d'un utilisateur", async () => {
  ...
  await userRepository.updateNames({...user1, lastName: newLastName, firstName: newFirstName})

  const user1InDb = await kysely.selectFrom('users').selectAll().where('email', '=', user1.email).executeTakeFirst()
  expect(user1InDb).toMatchObject({lastName: newLastName, firstName: newFirstName})

  const user2InDb = await kysely.selectFrom('users').selectAll().where('email', '=', user2.email).executeTakeFirst()
