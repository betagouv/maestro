...
export const NotificationCategoryMessages = {
  Control: ({matrix}) => `Nouveau commentaire sur la matrice **${matrix.toLowerCase()}**`,
  Surveillance: ({matrix}) => `Nouveau commentaire sur la matrice **${matrix.toLowerCase()}**`,
...
} as const satisfies{ [category in NotificationCategory] : ((params: TemplateParams<category>) => string) }
...
type TemplateParams<
  ...
  U = Omit<
    z.infer<
      (typeof Templates)[(typeof categoryToEmailTemplate)[T]]['params']
    >,
    'link'
  >
