...
beforeEach(() => {
  vi.resetAllMocks()
  spyDeleteDocument = vi.spyOn(s3Service, 'deleteDocument').mockResolvedValue(undefined)
  documentId = uuidv4()
  spyUploadDocument = vi.spyOn(s3Service, 'uploadDocument').mockResolvedValue({documentId, valid: true})
})
...
test("Le fichier est updloadé sur le S3, n'est pas supprimé du S3 et est en bdd", async () => {
  ...
  const document = await kysely.selectFrom('documents').selectAll().where('id', '=', documentId).executeTakeFirst()
  expect(document?.kind).toBe('AnalysisReportDocument')

  const analysis = await kysely.selectFrom('analysis').where('id', '=', analysisId).selectAll().executeTakeFirst()
  expect(analysis?.reportDocumentId).toBe(documentId)

  const analysisResidue = await kysely.selectFrom('analysisResidues').where('analysisId', '=', analysisId).selectAll().execute()
  expect(analysisResidue).toHaveLength(1)
  expect(analysisResidue[0].reference).toBe(analysisToSave.residues[0].residue.reference)
})

test("Retourne une erreur si l'upload a échoué", async () => {

  const error = 'BOOM'
  spyUploadDocument = vi.spyOn(s3Service, 'uploadDocument').mockResolvedValue({error, valid: false})
  await expect( async () => analysisHandler({
    notes: '',
    pdfFile: new File([], 'fileName'),
    sampleReference: Sample13Fixture.reference,
    residues: []
  })).rejects.toThrowError(`Impossible d'uploader le PDF sur le S3: HTTP ${error}`)
...
test("Impossible d'ajouter une analyse à un échantillon avec déjà une analyse", async () => {
  ...
  await expect(async () =>  analysisHandler(analysis)).rejects.toThrowErrorMatchingInlineSnapshot(`[Error: Une analyse est déjà présente pour cet échantillon : GES-08-24-315-A]`)
...
test("Impossible d'enregistrer l'analyse si on trouve une analyte qui ne correspond a aucun résidu complexe", async () => {
  ...
  })).rejects.toThrowErrorMatchingInlineSnapshot(`[Error: Impossible de trouver le résidu complexe pour l'analyte RF-0006-001-PPP]`)
...
test("Si une erreur intervient après l'upload sur le S3, on supprime le document du S3", async () => {
  ...
  })).rejects.toThrowErrorMatchingInlineSnapshot(`[error: invalid input syntax for type uuid: ""]`)
...
test("Peut enregistrer une analyse avec un résidue complexe et ses analytes associées", async () => {
  ...
  const analysisResidue = await kysely.selectFrom('analysisResidues').where('analysisId', '=', analysisId).selectAll().execute()
  expect(analysisResidue).toHaveLength(1)
  expect(analysisResidue[0].reference).toBe(analysisToSave.residues[1].residue.reference)

  const analysisResidueAnalytes = await kysely.selectFrom('residueAnalytes')
    .selectAll()
    .where('analysisId', '=', analysisId)
    .orderBy('analyteNumber asc')
    .execute()
  expect(analysisResidueAnalytes).toHaveLength(2)
  expect(analysisResidueAnalytes[0].reference).toBe(analysisToSave.residues[0].residue.reference)
  expect(analysisResidueAnalytes[1].reference).toBe(analysisToSave.residues[2].residue.reference)
})
