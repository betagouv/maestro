...
const generateAnalysisRequestCsv = async (data: AnalysisRequestData) => {
  const rows = [
    ...
    `Analyses mono-résidu;${data.monoSubstances?.map((substance) => `${substance.label};${substance.code}`).join(';')}`,
    `Analyses multi-résidus dont;${data.multiSubstances?.map((substance) => `${substance.label};${substance.code}`).join(';')}`,
