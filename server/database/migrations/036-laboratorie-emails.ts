...
export const up = async (knex: Knex) => {
  await knex.raw(
    `alter table public.laboratories add emails varchar(255)[] default '{}' not null;`
  );
  await knex.raw(
    `alter table public.laboratories add emails_analysis_result varchar(255)[] default '{}' not null;`
  );

  await knex('laboratories').update({ emails: knex.raw(`ARRAY[email]`) });
  await knex('laboratories').update({
    emails_analysis_result: knex.raw(`ARRAY[email_analysis_result]`)
  });
