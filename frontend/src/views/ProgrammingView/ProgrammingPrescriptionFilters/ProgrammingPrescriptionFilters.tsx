...
const ProgrammingPrescriptionFilters = ({
  ...
  const domains = useMemo(
    () =>
      (allDomains ?? []).filter(
        (domain) =>
          domain.year === filters.year &&
          options.plans.some((plan) => plan.domainId === domain.id)
      ),
    [allDomains, filters.year, options.plans]
  );

  const matrixKindOptions = useMemo(
    () =>
      sortBy(
        options.matrixKinds.map((kind) => ({
  ...
  const stageCoordinators = useMemo(
    () =>
      (coordinators ?? []).filter(
        (coordinator) =>
          !filters.stage || coordinator.stages.includes(filters.stage)
      ),
    ...
    <>
      <div className={filterClassName}>
        <AppCheckboxSelect
          ...
          searchable
          disabled={options.programmingSubPlanIds.length === 0}
        />
      </div>
      <div className={filterClassName}>
        <AppCheckboxSelect
      ...
      <div className={filterClassName}>
        <AppCheckboxSelect
          ...
          selectedValues={filters.matrixKinds ?? []}
          onChange={(matrixKinds) =>
            onChange({ matrixKinds: matrixKinds as MatrixKind[] })
          }
          emptyLabel="Toutes"
          summaryLabel="matrice"
          searchable
          disabled={matrixKindOptions.length === 0}
        />
      </div>
    </>
  );
      ...
      {canFilterByCoordinator && (
        <div className={filterClassName}>
          <AppCheckboxSelect
            ...
            searchable
            disabled={stageCoordinators.length === 0}
          />
        </div>
      )}
      {hasDepartmentalView && (
  ...
  const soleOptionKeys = [
    ...(options.plans.length <= 1 ? (['programmingPlanIds'] as const) : []),
    ...(options.programmingSubPlanIds.length <= 1
      ? (['programmingSubPlanIds'] as const)
      : []),
    ...(options.contexts.length <= 1 ? (['contexts'] as const) : [])
  ];
