...
import { isNil, max, omitBy } from 'lodash-es';
import {
  type Department,
  DepartmentLabels
...
const ProgrammingView = () => {
  ...
  useEffect(
    () => () => {
      const pending = changesToMarkViewedRef.current;
      changesToMarkViewedRef.current = null;
      if (pending?.prescriptionIds.length) {
        markLocalPrescriptionChangesViewed(pending);
      }
    },
                        ...
                        {selectedTabId === 'ProgrammationTab' &&
                          filteredProgrammingPlans.length > 0 && (
                            <ProgrammingPrescriptionList
                              ...
                              yearProgrammingPlans={filterablePlans}
