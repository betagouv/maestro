...
describe('computeDisplayStatus — ReadyToSend/modified branch', () => {
  ...
  test('National, viewed by Admin, still InProgress (nothing sent yet) -> InProgress, not ReadyToSend', () => {
    const result = computeDisplayStatus({
      ...base,
      status: 'InProgress',
      sentAt: null,
  ...
  test('National, viewed by Admin, InProgress and complete -> InProgress, jamais ReadyToSend', () => {
    const result = computeDisplayStatus({
      ...base,
      isComplete: true,
      status: 'InProgress',
